...
CURRICULUM_FILE = _path_service.get_settings_file("antitao_curriculum")


DEFAULT_ANTITAO_CURRICULUM: dict[str, Any] = {
    "version": 1,
    "tracks": [
        {
            "id": "seller",
            ...
            "actions": [
                {
                    ...
                    "prompt": {
                        "zh": "请把反向海淘讲给一个完全不懂的新手卖家听，重点解释它是什么、和普通跨境电商有什么区别、卖家/平台/代理/用户分别扮演什么角色，并用一个简单例子帮助理解。",
            ...
            "gates": [
                ...
                },
                {
                    ...
                    "pass_standard": {
                        "zh": "能画出用户购买流程，并解释 Weidian、淘宝、1688、店铺和代理平台之间的关系。",
            ...
            "gates": [
                {
                    ...
                    "common_questions": [
                        {
                            "zh": "反向海淘这个行业到底是怎么运转的？",
                            "en": "How does the AntiTao industry actually work?",
                        },
                        {
                            "zh": "平台运营和卖家视角最大的区别是什么？",
                            "en": "What is the biggest difference between the platform-ops view and the seller view?",
                        },
                        {
                            "zh": "平台在这门生意里真正提供的价值是什么？",
                            "en": "What real value does the platform provide in this business?",
                        },
                    ],
                    "pass_standard": {
                ...
                },
                {
                    ...
                    "common_questions": [
                        {
                            "zh": "用户从看到内容到下单收货的完整流程是什么？",
                            "en": "What is the complete flow from seeing content to receiving the order?",
                        },
                        {
                            "zh": "平台在哪些环节影响用户体验和转化？",
                            "en": "At which stages does the platform affect experience and conversion?",
                        },
                        {
                            "zh": "用户为什么会流失，常见卡点在哪里？",
                            "en": "Why do users drop off, and where are the usual friction points?",
                        },
                    ],
                    "pass_standard": {
                ...
                },
                {
                    ...
                    "common_questions": [
                        {
                            "zh": "商品是怎么进入平台体系的？",
                            "en": "How do products enter the platform system?",
                        },
                        {
                            "zh": "仓库、订单、上架、发货这些流程之间怎么衔接？",
                            "en": "How do warehousing, orders, listing, and shipping connect?",
                        },
                        {
                            "zh": "平台后台系统到底是为了解决什么问题？",
                            "en": "What problems is the platform backend system actually solving?",
                        },
                    ],
                    "pass_standard": {
                ...
                },
                {
                    ...
                    "common_questions": [
                        {
                            "zh": "平台为什么要和网红合作，而不是只做站内运营？",
                            "en": "Why does a platform need influencer cooperation instead of only doing internal operations?",
                        },
                        {
                            "zh": "平台和达人合作的基本流程是什么？",
                            "en": "What is the basic workflow for platform-influencer cooperation?",
                        },
                        {
                            "zh": "推广合作怎么从曝光走到平台承接？",
                            "en": "How does a promotion partnership move from exposure into platform conversion?",
                        },
                    ],
                    "pass_standard": {
                ...
                },
                {
                    ...
                    "common_questions": [
                        {
                            "zh": "平台为什么不能只看订单和 GMV？",
                            "en": "Why can’t a platform focus only on orders and GMV?",
                        },
                        {
                            "zh": "财务模型、推广模型、支付封控模型、系统模型分别在管什么？",
                            "en": "What do the finance, promotion, payment control, and system models each govern?",
                        },
                        {
                            "zh": "平台增长和平台风险为什么总是绑在一起？",
                            "en": "Why are platform growth and platform risk always tied together?",
                        },
                    ],
                    "pass_standard": {
...
def _normalize_text_block(value: Any) -> dict[str, str]:
...
def _normalize_track(track: dict[str, Any]) -> dict[str, Any]:
    gates = []
    for index, gate in enumerate(track.get("gates", []) or []):
        if not isinstance(gate, dict):
            continue
        common_questions = [
            _normalize_text_block(item)
            for item in (gate.get("common_questions", []) or [])
            if str(item or "").strip() or isinstance(item, dict)
        ]
        gates.append(
            {
                "id": str(gate.get("id") or f"{track.get('id', 'track')}-gate-{index + 1}"),
                "title": _normalize_text_block(gate.get("title")),
                "description": _normalize_text_block(gate.get("description")),
                "common_questions": common_questions[:10],
                "pass_standard": _normalize_text_block(gate.get("pass_standard")),
            }
        )

    actions = []
    for index, action in enumerate(track.get("actions", []) or []):
        if not isinstance(action, dict):
            continue
        capability = str(action.get("capability") or "").strip()
        actions.append(
            {
                "id": str(action.get("id") or f"{track.get('id', 'track')}-action-{index + 1}"),
                "title": _normalize_text_block(action.get("title")),
                "description": _normalize_text_block(action.get("description")),
                "prompt": _normalize_text_block(action.get("prompt")),
                "capability": "deep_research" if capability == "deep_research" else "",
            }
        )

    return {
        "id": str(track.get("id") or "seller"),
        "badge": _normalize_text_block(track.get("badge")),
        "title": _normalize_text_block(track.get("title")),
        "subtitle": _normalize_text_block(track.get("subtitle")),
        "summary": _normalize_text_block(track.get("summary")),
        "outcome": _normalize_text_block(track.get("outcome")),
        "actions": actions,
        "gates": gates,
    }


def normalize_antitao_curriculum(payload: dict[str, Any] | None) -> dict[str, Any]:
    raw = payload if isinstance(payload, dict) else {}
    tracks = [_normalize_track(track) for track in raw.get("tracks", []) or [] if isinstance(track, dict)]

    normalized = deepcopy(DEFAULT_ANTITAO_CURRICULUM)
    normalized["tracks"] = tracks or deepcopy(DEFAULT_ANTITAO_CURRICULUM["tracks"])
    normalized["updated_at"] = datetime.now().isoformat()
    return normalized


def load_antitao_curriculum() -> dict[str, Any]:
    if CURRICULUM_FILE.exists():
        try:
            with open(CURRICULUM_FILE, encoding="utf-8") as handle:
                saved = json.load(handle)
            return normalize_antitao_curriculum(saved)
        except Exception:
            return deepcopy(DEFAULT_ANTITAO_CURRICULUM)
    return deepcopy(DEFAULT_ANTITAO_CURRICULUM)


def save_antitao_curriculum(payload: dict[str, Any]) -> dict[str, Any]:
    curriculum = normalize_antitao_curriculum(payload)
    CURRICULUM_FILE.parent.mkdir(parents=True, exist_ok=True)
    with open(CURRICULUM_FILE, "w", encoding="utf-8") as handle:
        json.dump(curriculum, handle, ensure_ascii=False, indent=2)
    return curriculum
