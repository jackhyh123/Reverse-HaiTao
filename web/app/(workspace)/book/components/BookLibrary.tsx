...
> = {
  draft: {
    labelKey: "book.status.draft",
    className:
      "bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-300",
    dot: "bg-amber-500",
  },
  spine_ready: {
    labelKey: "book.status.outline",
    className: "bg-sky-50 text-sky-700 dark:bg-sky-500/10 dark:text-sky-300",
    dot: "bg-sky-500",
  },
  compiling: {
    labelKey: "book.status.compiling",
    className:
      "bg-violet-50 text-violet-700 dark:bg-violet-500/10 dark:text-violet-300",
    dot: "bg-violet-500 animate-pulse",
  },
  ready: {
    labelKey: "book.status.ready",
    className:
      "bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300",
    dot: "bg-emerald-500",
  },
  error: {
    labelKey: "book.status.error",
    className:
      "bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-300",
    dot: "bg-rose-500",
  },
  archived: {
    labelKey: "book.status.archived",
    className:
...
function relativeTime(seconds: number, t: (key: string, options?: Record<string, unknown>) => string): string {
  if (!seconds || Number.isNaN(seconds)) return "";
  const diff = Date.now() / 1000 - seconds;
  if (diff < 60) return t("book.time.justNow");
  const mins = Math.floor(diff / 60);
  if (mins < 60) return t("book.time.minutesAgo", { count: mins });
  const hrs = Math.floor(mins / 60);
  if (hrs < 24) return t("book.time.hoursAgo", { count: hrs });
  const days = Math.floor(hrs / 24);
  if (days < 30) return t("book.time.daysAgo", { count: days });
  const months = Math.floor(days / 30);
  if (months < 12) return t("book.time.monthsAgo", { count: months });
  return t("book.time.yearsAgo", { count: Math.floor(months / 12) });
}
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="text-sm font-semibold text-[var(--foreground)]">
              {t("book.library.title")}
            </div>
            <div className="text-xs text-[var(--muted-foreground)]">
              {t("book.library.subtitle")}
            </div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative hidden items-center sm:flex">
            ...
            <input
              ...
              placeholder={t("book.library.searchPlaceholder")}
      ...
      <main className="flex-1 overflow-y-auto px-6 py-6">
        ...
        <div className="mb-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          <StatCard
            ...
            label={t("book.library.totalBooks")}
            value={stats.total}
          />
          <StatCard
            icon={<Sparkles size={14} />}
            label={t("book.library.ready")}
            value={stats.ready}
            accent="text-emerald-600 dark:text-emerald-400"
          />
          <StatCard
            icon={<Loader2 size={14} />}
            label={t("book.library.inProgress")}
            value={stats.inProgress}
            accent="text-violet-600 dark:text-violet-400"
          />
          <StatCard
            icon={<Layers size={14} />}
            label={t("book.library.chapters")}
        ...
        <div className="mb-3 flex items-end justify-between">
          <div>
            <div className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[var(--muted-foreground)]">
              {t("book.library.myLibrary")}
            </div>
            <div className="text-xs text-[var(--muted-foreground)]/80">
              {t("book.library.count", {
                shown: filtered.length,
                total: books.length,
              })}
              {query ? ` · ${t("book.library.matching", { query })}` : ""}
            </div>
          </div>
        </div>
                ...
                >
                  ...
                  >
                    ...
                    <button
                      ...
                      title={
                        isPendingDelete
                          ? t("book.library.clickAgainToConfirm")
                          : t("book.library.deleteBook")
                      }
    ...
    <div className="flex flex-col items-center justify-center gap-4 rounded-xl border border-dashed border-[var(--border)] bg-[var(--secondary)]/30 px-8 py-16 text-center">
      ...
      <div>
        <p className="text-base font-medium text-[var(--foreground)]">
          {t("book.library.emptyTitle")}
        </p>
        <p className="mt-1 text-sm text-[var(--muted-foreground)]">
          {t("book.library.emptySubtitle")}
        </p>
      </div>
      <button
        type="button"
